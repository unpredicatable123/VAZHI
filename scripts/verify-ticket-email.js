...
async function projectId() {
    ...
    if (!value)
        throw new Error('No project in .firebaserc; pass --project=<id>.');
    return value;
}
...
async function main() {
    if (!process.env.GOOGLE_APPLICATION_CREDENTIALS) {
        throw new Error('Set GOOGLE_APPLICATION_CREDENTIALS to an Admin service-account key first. See FIREBASE.md.');
    }
    ...
    const recipients = Array.isArray(queued.to) ? queued.to.map(String) : [];
    check('addressed to exactly one recipient', recipients.length === 1, String(recipients.length));
    check('THE RECIPIENT IS THE SIGN-IN EMAIL', recipients[0] === signInEmail, `${mask(recipients[0] ?? '')} vs ${mask(signInEmail)}`);
    /* ---- the message itself ---- */
    ...
    if (!delivery) {
        ...
        if (delivery.info?.accepted?.length) {
            console.log(`  accepted by the server for: ${delivery.info.accepted.map((a) => mask(a)).join(', ')}`);
        }
        if (delivery.expireAt)
            console.log('  TTL stamp present on the queued message');
    }
