...
import { demoAccounts, type DemoAccount } from '../src/lib/mocks/accounts.mock';
...
function firebaseEmail(account: DemoAccount): string {
...
async function authenticate(account: DemoAccount): Promise<void> {
...
async function signOutAndVerify(): Promise<void> {
	await signOut(auth);
	if (auth.currentUser !== null) throw new Error('Firebase Auth still has a current user after sign-out.');
}
...
async function main(): Promise<void> {
	...
	const search = await httpsCallable<
		{ serviceDate: string },
		{ trips: Array<{ id: string; routeId: string }> }
	>(functions, 'searchTrips')({ serviceDate });
	if (!search.data.trips.length) throw new Error('Public search returned no seeded trips.');
	for (const routeId of requestedRouteIds) {
		const count = search.data.trips.filter((trip) => trip.routeId === routeId).length;
		if (count < 2) throw new Error(`${routeId} returned only ${count} seeded trips.`);
	}
	...
	for (const account of demoAccounts) {
		...
		if (account.role === 'traveller') {
			await httpsCallable<{ tripId: string }, { blockedSeatIds: string[] }>(
				functions,
				'getSeatAvailability'
			)({ tripId: search.data.trips[0].id });
		} else if (account.role === 'driver' || account.role === 'conductor') {
			if (!account.driverId) throw new Error(`${account.role} demo badge is missing.`);
			await httpsCallable<{ badgeId: string }, { valid: true }>(functions, 'verifyDutyIdentity')({
				badgeId: account.driverId
			});
			await httpsCallable<Record<string, never>, { trip: { id: string } }>(
				functions,
				'getAssignedTrip'
			)({});
		} else {
			...
			for (const routeId of requestedRouteIds) {
				...
				if (routeTrips.length < 2) throw new Error(`Operations cannot see ${routeId}.`);
			...
			await httpsCallable<
				...
				{ conflicts: unknown[] }
			>(functions, 'validateTripAssignment')({
				trip: {
					serviceDate,
					busId: 'SMOKE-BUS',
...
try {
	...
	if (auth.currentUser) await signOutAndVerify();
	await deleteApp(app);
}
