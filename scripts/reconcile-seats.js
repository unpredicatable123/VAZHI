...
async function main() {
    ...
    for (const doc of bookings.docs)
        liveBookings.set(doc.id, String(doc.data().status ?? ''));
    ...
    for (const trip of trips.docs) {
        ...
        for (const seat of seats.docs) {
            ...
            if (state === 'held') {
                const holdLive = value.expiresAt && typeof value.expiresAt.toMillis === 'function'
        ...
        if (stale.length === 0 && !drifted)
            continue;
        console.log(`${trip.id}`);
        if (stale.length)
            console.log(`  free up ${stale.length} seat(s): ${stale.join(', ')}`);
        if (drifted)
            console.log(`  seatsAvailable ${isNow} -> ${shouldBe}`);
        if (apply) {
