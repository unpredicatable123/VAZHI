...
const { applicationDefault, deleteApp: deleteAdminApp, initializeApp: initializeAdminApp } =
...
if (!process.env.GOOGLE_APPLICATION_CREDENTIALS) {
	throw new Error(
		'GOOGLE_APPLICATION_CREDENTIALS is required so temporary smoke-test accounts can be removed.'
	);
}
...
let travellerUid: string | undefined;
...
async function availableDutyId(): Promise<string> {
	for (let number = 999; number >= 900; number--) {
		...
		if (!record.exists && !accountExists) return candidate;
	}
...
async function main(): Promise<void> {
	...
	await httpsCallable<{ displayName: string }, { registered: true }>(functions, 'registerTraveller')({
	...
	const created = await httpsCallable<
		...
		{ credentials: { identifier: string; badgeId: string } }
	...
	await signInWithEmailAndPassword(
		auth,
		`${credentials.identifier.toLowerCase()}@crew.vazhi.app`,
		travellerPassword
	);
	...
	if (!auth.currentUser?.email) throw new Error('Provisioned driver has no Firebase email.');
	await reauthenticateWithCredential(
		auth.currentUser,
		EmailAuthProvider.credential(auth.currentUser.email, travellerPassword)
	);
	...
	console.log(
		'Account provisioning smoke tests passed: traveller registration, Operations password, crew login, crew password change.'
	);
}
...
try {
	...
	if (travellerUid) await adminAuth.deleteUser(travellerUid).catch(() => undefined);
	if (dutyId) {
		const crewAccount = await adminAuth
			.getUserByEmail(`${dutyId.toLowerCase()}@crew.vazhi.app`)
			.catch(() => undefined);
		if (crewAccount) await adminAuth.deleteUser(crewAccount.uid).catch(() => undefined);
