...
function projectFromArguments(): string | undefined {
	const equalsArgument = process.argv.find((argument) => argument.startsWith('--project='));
	if (equalsArgument) return equalsArgument.slice('--project='.length);

	const projectIndex = process.argv.indexOf('--project');
	return projectIndex >= 0 ? process.argv[projectIndex + 1] : undefined;
}

function projectFromFirebaseConfig(): string | undefined {
	if (!process.env.FIREBASE_CONFIG?.startsWith('{')) return undefined;
	try {
...
async function resolveProjectId(): Promise<string> {
	...
	try {
		...
		if (defaultProject && defaultProject !== 'your-firebase-project-id') return defaultProject;
	} catch (error) {
	...
	throw new Error(
		'Firebase project ID is missing. Set projects.default in .firebaserc or run npm run firebase:seed -- --project=YOUR_PROJECT_ID.'
	);
}
...
try {
	...
	throw new Error(
		'Firebase Admin credentials are missing. Set GOOGLE_APPLICATION_CREDENTIALS to a Firebase service-account JSON file stored outside this repository, then run the seed again.',
		{ cause: error }
	);
}
...
function authEmail(identifier: string, role: string): string {
...
for (const account of demoAccounts) {
	...
	catch { user = await getAuth().createUser({ email, password: account.password, displayName: account.displayName }); }
	...
	if (account.role === 'traveller') demoTravellerUid = user.uid;
...
if (demoTravellerUid) {
	...
	const byPnr = new Map<string, typeof entries>();
	for (const entry of entries) byPnr.set(entry.pnr, [...(byPnr.get(entry.pnr) ?? []), entry]);
