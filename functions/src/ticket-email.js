...
const PAPER = '#fafaf7';
const SOFT = '#e6efe8';
const FONT = "-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Noto Sans Tamil',Helvetica,Arial,sans-serif";
...
function label(english, tamil) {
    return (`<span style="display:block;font-size:11px;line-height:1.4;letter-spacing:.08em;` +
...
function row(english, tamil, value, mono = false) {
 ...
 */
export function ticketEmailHtml(booking) {
 ...
 * here too.
 */
export function ticketEmailText(booking) {
    const lines = [
    ...
    lines.push(``, `Base fare: ${rupees(booking.fare.baseFare)}`, `Taxes: ${rupees(booking.fare.taxes)}`, `Total paid: ${rupees(booking.fare.total)} (${booking.paymentMethod})`, `Demonstration payment — no money moved.`, ``, `Show your booking reference when boarding. Carry a photo ID matching the`, `passenger name. Boarding closes 10 minutes before departure.`, ``, `Sent to the address on your VAZHI account. We never ask for card details,`, `OTPs, or passwords by email.`);
    return lines.join('\n');
}
/** Subject line: recognisable in a crowded inbox without being opened. */
export function ticketEmailSubject(booking) {
