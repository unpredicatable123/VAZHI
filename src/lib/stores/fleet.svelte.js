...
function load() {
    const empty = { buses: [], crew: [], retiredBusIds: [], retiredCrewIds: [] };
    if (!browser)
        return empty;
    try {
        ...
        return {
            ...
            crew: Array.isArray(parsed.crew)
                ? parsed.crew
                    ...
                    .map((entry) => serialiseCrew(entry))
                : [],
            retiredBusIds: Array.isArray(parsed.retiredBusIds)
                ? parsed.retiredBusIds.filter((id) => typeof id === 'string')
                : [],
            retiredCrewIds: Array.isArray(parsed.retiredCrewIds)
                ? parsed.retiredCrewIds.filter((id) => typeof id === 'string')
                : []
        };
    }
    catch {
...
class FleetStore {
    ...
    get buses() {
        ...
        const base = busFleetFixtures.filter((bus) => !retired.has(bus.id) && !editedIds.has(bus.id));
        ...
        return [...base, ...edits].sort((a, b) => (fixtureOrder.get(a.id) ?? Number.MAX_SAFE_INTEGER) -
    ...
    get crew() {
        ...
        const base = crewFixtures.filter((member) => !retired.has(member.id) && !editedIds.has(member.id));
