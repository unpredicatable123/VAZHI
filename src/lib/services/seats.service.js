 ...
 */
export async function getSeatDeck(busId) {
    await simulateLatency();
    const fixture = seatDeckFixtures[busId];
    if (fixture)
        return { status: 'ok', data: await overlaySharedAvailability(busId, fixture) };
    const service = findService(busId);
    if (!service) {
        return { status: 'error', error: { code: 'not_found', messageKey: 'seats_error_body' } };
    }
    const bus = busFleetFixtures.find((candidate) => candidate.registrationNumber === service.vehicleNumber);
...
async function overlaySharedAvailability(tripId, deck) {
    try {
        const { functions } = requireFirebase();
        const response = await httpsCallable(functions, 'getSeatAvailability')({ tripId });
...
function matchesSeatType(seat, seatType) {
    if (seatType === 'window')
        return seat.signals.window;
    if (seatType === 'aisle')
        return seat.signals.aisle;
    return true;
}
/**
 ...
 */
export function recommendSeats(deck, preferences, count, accessibleTravelMode) {
    const ranked = availableSeats(deck)
        ...
        if (a.matchesType !== b.matchesType)
            return a.matchesType ? -1 : 1;
        if (b.score !== a.score)
...
function scoreSeat(seat, preferences, accessibleTravelMode) {
    let score = 0;
    if (accessibleTravelMode) {
        if (seat.signals.accessible)
            score += 100;
        if (seat.signals.nearEntrance)
            score += 40;
    }
    ...
    if (preferences.assistance.includes('visual') && seat.signals.nearEntrance)
        score += 15;
    if (preferences.assistance.includes('hearing') && seat.signals.quieter)
        score += 15;
    ...
    if (seat.signals.quieter)
        score += 10;
    if (seat.signals.aisle)
        score += 2;
    return score;
}
/** Visual state for a seat, derived from availability and current selection. */
export function seatState(seat, selected, recommended) {
    if (selected.includes(seat.id))
        return 'selected';
    if (seat.availability === 'unavailable')
        return 'unavailable';
    if (recommended.includes(seat.id))
        return 'recommended';
    return 'available';
}
/**
 ...
 */
export function rowTraits(deck, row) {
    return {
        extraLegroom: row === 1,
 ...
 */
export function zoneBands(deck) {
    const bands = [];
    for (const id of ['entrance', 'quieter']) {
        const rowsWith = [];
