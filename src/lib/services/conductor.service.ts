...
import type { Bus, TransitRoute, Trip, TripStatus as FleetTripStatus, TripView } from '$types/fleet';
...
async function currentTripId(): Promise<string> {
	if (assignedTripId) return assignedTripId;
	const { functions } = requireFirebase();
	const assigned = await httpsCallable<Record<string, never>, { trip: Trip }>(
		functions,
		'getAssignedTrip'
	)({});
...
export async function getAssignment(
	conductorId: string
): Promise<ServiceResult<ConductorAssignment>> {
	try {
		const { db, functions } = requireFirebase();
		const assigned = await httpsCallable<Record<string, never>, { trip: Trip }>(functions, 'getAssignedTrip')({});
		...
		if (!busDoc.exists() || !routeDoc.exists()) return { status: 'error', error: { code: 'not_found', messageKey: 'assignment_none_body' } };
		const bus = { id: busDoc.id, ...busDoc.data() } as Bus;
		const route = { id: routeDoc.id, ...routeDoc.data() } as TransitRoute;
		const view: TripView = {
			trip, bus, route,
...
export async function getManifest(): Promise<ServiceResult<ManifestEntry[]>> {
	try {
...
function manifestFromData(entry: DocumentData, groupSize: number): ManifestEntry {
	return {
...
export function totalsFor(entries: ManifestEntry[], capacity: number): BoardingTotals {
 ...
 */
export async function verifyPnr(pnr: string): Promise<ServiceResult<VerificationResult>> {
	...
	try {
		const { functions } = requireFirebase();
		const response = await httpsCallable<{ pnr: string }, VerificationResult>(functions, 'verifyPnr')({ pnr: trimmed });
...
export async function markBoarded(pnr: string): Promise<ServiceResult<ManifestEntry[]>> {
	return updateBoarding(pnr, 'boarded');
}

export async function markPending(pnr: string): Promise<ServiceResult<ManifestEntry[]>> {
	return updateBoarding(pnr, 'pending');
}

async function updateBoarding(pnr: string, status: 'pending' | 'boarded'): Promise<ServiceResult<ManifestEntry[]>> {
	try {
		const { db } = requireFirebase();
		const tripId = await currentTripId();
 ...
 */
export function seatBoardingState(
	seatId: string,
	deck: SeatDeck | null,
	...
	if (entry.ticketStatus === 'cancelled') return 'cancelled';
