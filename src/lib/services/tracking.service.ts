...
import type { Booking, TrackingSnapshot, TrackingStop } from '$types/booking';
 ...
 */
function journeyProgress(booking: Booking, now: Date): number {
...
function positionAlong(geometry: RouteGeometry, fraction: number): [number, number] {
	const line = geometry.lines[0]?.geometry.coordinates ?? [];
	if (line.length === 0) return [0, 0];
	if (line.length === 1 || fraction <= 0) return line[0] as [number, number];
	if (fraction >= 1) return line[line.length - 1] as [number, number];
 ...
 */
function buildStops(booking: Booking, geometry: RouteGeometry, progress: number): TrackingStop[] {
	...
	if (stops.length > 0) stops[0].name = booking.originName;
	if (stops.length > 1) stops[stops.length - 1].name = booking.destinationName;
...
export async function getTrackingSnapshot(
	...
	now: Date = new Date()
): Promise<ServiceResult<TrackingSnapshot>> {
	...
	const geometryResult = await getRouteGeometry(
		routeIdForJourney(booking.originStopId, booking.destinationStopId),
		fetcher
	);
