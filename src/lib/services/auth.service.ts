...
export function requiresDutyId(role: UserRole): boolean {
	return role === 'conductor' || role === 'driver';
}

export interface TravellerRegistration {
	displayName: string;
	email: string;
	password: string;
}
...
function firebaseIdentifier(identifier: string, role: UserRole): string {
...
export function observeSession(callback: (value: Session | null) => void): () => void {
...
export async function signIn(
	...
	try {
		...
		const credential = await signInWithEmailAndPassword(
			sdk.auth,
			firebaseIdentifier(identifier, role),
			credentials.password
		);
		const resolved = await sessionFromUser(credential.user, true);
		if (!resolved || resolved.role !== role) throw new Error('auth/role-mismatch');
		if (requiresDutyId(role)) {
			await httpsCallable<{ badgeId: string }, { valid: true }>(sdk.functions, 'verifyDutyIdentity')({ badgeId });
		}
...
export async function registerTraveller(
	registration: TravellerRegistration
): Promise<ServiceResult<Session>> {
	...
	try {
		...
		await httpsCallable<{ displayName: string }, { registered: true }>(
			sdk.functions,
			'registerTraveller'
		)({ displayName });
		roleAssigned = true;
		const resolved = await sessionFromUser(createdUser, true);
		if (!resolved || resolved.role !== 'traveller') throw new Error('auth/role-mismatch');
		return { status: 'ok', data: resolved };
	} catch (error) {
		if (createdUser && !roleAssigned) await deleteUser(createdUser).catch(() => undefined);
		if (auth?.currentUser) await firebaseSignOut(auth).catch(() => undefined);
...
export async function changeCurrentUserPassword(
	...
	try {
		...
		await reauthenticateWithCredential(
			user,
			EmailAuthProvider.credential(user.email, passwords.currentPassword)
		);
