...
export const filterKeys = ['time', 'price', 'ac', 'access'];
/** The coach-type choices, in order. */
export const coachOptions = ['all', 'seater', 'sleeper'];
/** Whether a service is a sleeper. One place decides, so nothing drifts. */
function isSleeper(bus) {
    return bus.cabinClass === 'sleeper';
}
export function totalFare(bus) {
 ...
 */
export function fareThreshold(results) {
    if (results.length === 0)
        return 0;
    /*
...
export async function searchBuses(criteria) {
    ...
    try {
        ...
        const [tripResponse, busDocs, routeDocs] = await Promise.all([
            httpsCallable(requireFirebase().functions, 'searchTrips')({ serviceDate: criteria.date }),
            getDocs(collection(db, 'buses')),
            getDocs(collection(db, 'routes'))
        ]);
        const buses = new Map(busDocs.docs.map((entry) => [entry.id, ({ id: entry.id, ...entry.data() })]));
        const routes = new Map(routeDocs.docs.map((entry) => [entry.id, ({ id: entry.id, ...entry.data() })]));
 ...
 */
function servesCorridor(bus, criteria) {
    ...
    if (!wantedOrigin || !wantedDestination)
        return false;
    return (districtIdForStop(bus.originStopId) === wantedOrigin &&
...
export function applyFilters(results, filters) {
    ...
    return results.filter((bus) => {
        if (filters.time && !isMorningDeparture(bus))
            return false;
        if (filters.price && totalFare(bus) > priceCut)
            return false;
        if (filters.ac && !bus.amenities.airConditioned)
            return false;
        if (filters.coach === 'seater' && isSleeper(bus))
            return false;
        if (filters.coach === 'sleeper' && !isSleeper(bus))
            return false;
        if (filters.access && !bus.accessibleBoardingPoint)
            return false;
        return true;
    });
}
export function filterOutcomes(results, filters) {
    const outcomes = {};
 ...
 */
export function coachOutcomes(results, filters) {
    const outcomes = {};
    const current = applyFilters(results, filters).length;
...
function isMorningDeparture(bus) {
...
function prioritiseAccessible(results) {
    return [...results].sort((a, b) => {
        if (a.canonical !== b.canonical)
            return a.canonical ? -1 : 1;
