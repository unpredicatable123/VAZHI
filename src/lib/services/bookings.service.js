...
function withValidDuration(booking) {
    if (Number.isFinite(booking.durationMinutes) && booking.durationMinutes > 0)
        return booking;
...
export async function getBooking(pnr) {
    ...
    try {
        ...
        if (snapshot.exists()) {
            const b = withValidDuration(snapshot.data());
...
export async function listTrips() {
    try {
        ...
        const snapshots = await getDocs(query(collection(db, 'bookings'), where('travellerId', '==', auth.currentUser.uid), orderBy('createdAt', 'desc')));
        return { status: 'ok', data: snapshots.docs.map((entry) => withValidDuration(entry.data())) };
    }
    catch (error) {
        return { status: 'error', error: mapFirebaseError(error, 'tracking_error_body') };
    }
}
export function filterTrips(trips, filter) {
...
export async function cancelBooking(pnr, booking) {
    try {
        ...
        try {
            const result = await httpsCallable(functions, 'cancelBooking')({ pnr });
