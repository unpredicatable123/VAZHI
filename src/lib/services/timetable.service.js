 ...
 * mapper.
 */
function deriveTrips(criteria) {
    const { originStopId, destinationStopId, date } = criteria;
    if (!isDerivableCorridor(originStopId, destinationStopId))
        return [];
    ...
    if (!origin || !destination)
        return [];
    const distanceKm = Math.max(MINIMUM_CORRIDOR_KM, Math.round(roadDistanceKm(origin.coordinates, destination.coordinates)));
    let runningMinutes = Math.round((distanceKm / AVERAGE_SPEED_KMH) * 60);
    if (distanceKm >= REST_STOP_THRESHOLD_KM)
        runningMinutes += REST_STOP_MINUTES;
