...
function corridorId(originStopId, destinationStopId) {
    return `${CORRIDOR_PREFIX}${originStopId}~${destinationStopId}`;
}
function parseCorridorId(routeId) {
    if (!routeId.startsWith(CORRIDOR_PREFIX))
        return null;
 ...
 */
function segmentId(routeId, originStopId, destinationStopId) {
...
function parseSegmentId(routeId) {
    if (!routeId.startsWith(SEGMENT_PREFIX))
        return null;
 ...
 */
export function routeIdForJourney(originStopId, destinationStopId) {
...
export function resolveRoute(routeId) {
    ...
    if (routeId.endsWith(REVERSED_SUFFIX)) {
        const base = routeFixtures.find((route) => route.id === routeId.slice(0, -REVERSED_SUFFIX.length));
        if (!base)
            return null;
    ...
    if (segment) {
        ...
        const distanceKm = wholeLength > 0
 ...
 */
export function roadDistanceKm(a, b, detour = DIRECT_DETOUR) {
 ...
 */
function withBounds(route, line) {
...
function geometryFromRoute(route) {
    return withBounds(route, route.stops.map((stop) => stop.coordinates));
}
/** Index of the polyline vertex closest to a point. */
function nearestVertex(line, point) {
    let best = 0;
 ...
 */
function sliceGeometry(base, route) {
    const line = (base.lines[0]?.geometry.coordinates ?? []);
...
export async function getRouteGeometry(routeId, fetcher = globalThis.fetch) {
    ...
    if (path) {
        try {
            const fetched = await fetchJson(path, fetcher);
            ...
            const geometry = {
                ...
                lines: fetched.lines.map((line) => ({
                    ...
                    geometry: {
                        ...line.geometry,
                        coordinates: smoothPath(line.geometry.coordinates)
                    }
                }))
            };
