 ...
 */
function findBusByRegistration(registrationNumber) {
...
function busIdFor(registrationNumber) {
    return `bus-${plateKey(registrationNumber).toLocaleLowerCase()}`;
}
export async function saveBus(draft) {
 ...
 */
function busCommitments(busId, trips) {
    return trips.filter((trip) => trip.busId === busId && trip.status !== 'cancelled');
}
export async function deleteBus(busId, trips) {
