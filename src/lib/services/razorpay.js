 ...
 */
export function openCheckout(request) {
    const keyId = import.meta.env.VITE_RAZORPAY_KEY_ID?.trim();
    if (!keyId)
        return Promise.resolve({ status: 'failed', reason: 'razorpay/not-configured' });
    ...
    if (!Checkout)
        return Promise.resolve({ status: 'failed', reason: 'razorpay/script-failed' });
    return new Promise((resolve) => {
        ...
        checkout.on('payment.failed', (payload) => settle({ status: 'failed', reason: payload?.error?.description }));
        checkout.open();
    });
}
