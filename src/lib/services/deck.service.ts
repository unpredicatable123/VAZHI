...
import type { SeatLayout } from '$types/transit';
...
function columnsFor(
	layout: SeatLayout,
	kind: DeckKind
): { left: SeatColumn[]; right: SeatColumn[] } {
	...
	if (layout === '2+1') return { left: ['A', 'B'], right: ['C'] };
	return kind === 'sleeper'
		? { left: ['A', 'B'], right: ['C', 'D'] }
		: { left: ['A', 'B'], right: ['C', 'D'] };
}
...
export interface DeckSpec {
	/** Service id — the plan is keyed and seeded by it. */
	serviceId: string;
	kind: DeckKind;
	layout: SeatLayout;
	/** Total capacity of the vehicle. */
	totalSeats: number;
	/** How many of them are still for sale. */
	seatsAvailable: number;
	/** Whether the vehicle has step-free boarding fitted. */
 ...
 */
export function buildDeck(spec: DeckSpec): SeatDeck {
	...
	*/
	...
	for (let row = 1; row <= rows; row++) {
		for (const column of [...left, ...right]) allIds.push(`${row}${column}`);
	}
	...
	for (let row = 1; row <= rows; row++) {
		for (const column of [...left, ...right]) {
			...
			const womenNearbyArea =
				row >= womenNearbyFrom && row <= womenNearbyTo && left.includes(column);
...
export function berthRows(deck: SeatDeck, level: BerthLevel): number[] {
	const rows = new Set<number>();
	for (const seat of deck.seats) {
		if (seat.berth === level) rows.add(seat.row);
	}
