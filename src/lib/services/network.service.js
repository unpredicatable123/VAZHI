 ...
 */
function nearestNetworkNode(point) {
    let best = null;
 ...
 */
export function waypointsBetween(from, to) {
    ...
    */
    const trimmed = [...path];
    if (squaredDistance(trimmed[0].coordinates, from) < SAME_PLACE_SQ)
        trimmed.shift();
    if (trimmed.length > 0 &&
 ...
 */
export function smoothPath(points, segmentsPerSpan) {
    if (points.length < 3)
        return [...points];
    /*
    ...
    */
    ...
    const density = segmentsPerSpan ?? Math.min(16, Math.max(2, Math.round(TARGET_VERTICES / spans)));
    /*
    ...
    */
    ...
    const out = [];
    const knot = (a, b, t) => t + Math.pow(Math.hypot(b[0] - a[0], b[1] - a[1]), 0.5);
