...
async function discardCachedData(): Promise<void> {
	...
	await Promise.race([
		clear,
		new Promise<void>((resolve) => setTimeout(resolve, CACHE_CLEAR_BUDGET_MS))
	]);
}
 ...
 */
export async function signOutAndRedirect(onbefore?: () => void): Promise<void> {
