...
/** Every topic Payani can speak to. The component renders one answer per id. */
export type PayaniTopic =
	| 'greeting'
	| 'book'
	| 'search'
	| 'seat'
...
export interface PayaniMatch {
	/** The topic to answer with. `null` means nothing matched well enough. */
	topic: PayaniTopic | null;
	/** Score of the winning topic. Exposed for tests and tuning. */
	score: number;
}
...
interface Topic {
	id: PayaniTopic;
	/** Scored by length: a longer phrase is a more specific signal. */
	keywords: string[];
	/**
	 * Short but unmistakable tokens, scored flat and high.
	 *
	 * "pnr" is three characters but means exactly one thing, so length is the
	 * wrong measure for it — unlike "bus" or "seat", which appear in half the
 ...
 */
export function matchTopic(question: string): PayaniMatch {
	const text = normalise(question);
	if (text === '') return { topic: null, score: 0 };
	...
	if (!isQuestion) {
		for (const phrase of ACTION_PHRASES) {
			if (text.includes(normalise(phrase))) return { topic: 'cannot', score: 100 };
		}
	}

	let best: PayaniTopic | null = null;
	let bestScore = 0;
	...
	for (const topic of TOPICS) {
		...
		for (const keyword of topic.strong ?? []) {
			...
			if (needle !== '' && new RegExp(`(^| )${needle}( |$)`).test(text)) score += STRONG_SCORE;
		}
		for (const keyword of topic.keywords) {
			const needle = normalise(keyword);
			if (needle !== '' && text.includes(needle)) score += needle.length;
		}
		if (score > bestScore) {
			bestScore = score;
			best = topic.id;
...
export const SUGGESTED_TOPICS: PayaniTopic[] = [
	'book',
	'pnr',
	'seat',
	'cancel',
