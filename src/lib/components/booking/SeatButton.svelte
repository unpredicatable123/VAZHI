<script lang="ts">
	...
	/**
	 * A single seat, drawn as a seat rather than a chip.
	 *
	 * Top-down coach view: the backrest sits on the rear edge and the armrests
	 * run down both sides, so every seat visibly faces the front of the bus.
	 *
	 * Colour carries STATE only — available, recommended, selected,
	 * unavailable — using the approved palette. Comfort signals are marked with
	 * small glyphs and by where the seat sits in the coach (window rail, zone
	 * bands, door), never by inventing a colour per signal. That keeps the deck
	 * readable and keeps state legible for anyone who cannot separate hues.
	 */
	...
	const accessibleName = $derived(
		[m.seat_label({ seat: seat.id }), stateLabel, ...signalLabels].join('. ')
	);
