<script lang="ts">
	...
	/**
	 * Camera QR scanning, for a conductor working the door on a phone.
	 *
	 * WHY IT DECODES TWO WAYS. `BarcodeDetector` is native, free, and fast, and
	 * it is what Chrome on Android — the overwhelming majority of depot phones —
	 * provides. iOS Safari has no such API, so `jsQR` is imported *dynamically*
	 * and only when the native path is missing: a browser that can decode
	 * natively never downloads the decoder at all.
	 *
	 * THE CAMERA IS A BORROWED RESOURCE. Every exit — a successful scan, the
	 * cancel button, the component being torn down — runs the same `stop()`, so
	 * the track is released and the phone's camera indicator goes out. A scanner
	 * that leaves the camera live on a shared depot device is not acceptable.
	 *
	 * PRIVACY: frames are decoded in memory and never leave the device. Nothing
	 * is recorded, uploaded, or kept — the video element is the only place a
	 * frame ever exists, and it is torn down with the stream.
	 */

	interface Props {
		/** Called with whatever the code contained, once, then the scanner closes. */
		onscan: (raw: string) => void;
		oncancel: () => void;
	}

	let { onscan, oncancel }: Props = $props();

	let video = $state<HTMLVideoElement | null>(null);
	let status = $state<'starting' | 'scanning' | 'denied' | 'unsupported' | 'failed'>('starting');
	...
	*/
	...
	async function decodeLoop(): Promise<void> {
	...
	async function nativeDetector(): Promise<{ detect(source: CanvasImageSource): Promise<{ rawValue: string }[]> } | null> {
		const Detector = (
