import type { RouteAccess, Session, UserRole } from '$types/auth';
import { roleHome } from '$types/auth';

/**
 * Route protection rules, in one place.
 *
 * Every page derives its access requirement from this table, so no component
 * ever implements its own authentication check and there is exactly one file
 * to review when the rules change.
 *
 * The landing page and the support pages stay public so a visitor can see what
 * VAZHI is. Searching for a service is the start of a booking, so `/explore`
 * and everything after it — seats, tickets, trips, refunds, account settings —
 * requires a traveller session. Each operational workspace requires its own
 * role: the conductor workspace admits only a conductor, the driver workspace
 * only a driver, and the operations workspace only a controller. A wrong-role
 * request is sent to that role's own home rather than to a sign-in screen.
 */
...
export type Redirect = { to: string; reason: 'sign-in-required' | 'wrong-role' } | null;
 ...
 */
export function redirectFor(pathname: string, current: Session | null): Redirect {
...
const signInPaths: Record<UserRole, string> = {
...
export function signInPathFor(role: UserRole): string {
...
export function isWorkspacePath(pathname: string): boolean {
	return ['/conductor', '/driver', '/operations'].some(
 ...
 */
export function withRedirectTo(signInPath: string, target: string): string {
	if (!target || target === '/') return signInPath;
