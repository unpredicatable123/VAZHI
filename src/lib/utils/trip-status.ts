...
import type { CrewStatus, TripStatus } from '$types/fleet';

/**
 * Labels and tones for the operational statuses, in one place.
 *
 * Every screen that shows a trip or duty status — the driver dashboard, the
 * conductor card, four Operations tables — reads its wording and its colour
 * from here, so a status cannot mean one thing on one screen and something
 * else on the next. Labels go through Paraglide, so both languages come for
 * free.
 */
 ...
 */
export function dutyStatusFrom(
	rosterStatus: CrewStatus,
	assignedTripStatus: TripStatus | undefined
): CrewStatus {
	if (rosterStatus === 'off-duty') return 'off-duty';
	if (!assignedTripStatus) return 'available';
	if (assignedTripStatus === 'boarding' || assignedTripStatus === 'departed') return 'on-trip';
	if (assignedTripStatus === 'in-transit') return 'on-trip';
	if (assignedTripStatus === 'completed' || assignedTripStatus === 'cancelled') return 'available';
	return 'assigned';
}
