<script lang="ts">
	...
	import ErrorSummary, { type SummaryIssue } from '$components/booking/ErrorSummary.svelte';
	...
	import type { PassengerFieldError } from '$types/booking';
	import type { PageData } from './$types';

	/**
	 * Passenger Details (specification section 10).
	 *
	 * One form per selected seat, with the seat association shown in each card
	 * heading. Every value stays in the in-memory passenger store: nothing is
	 * written to storage, the URL, cookies, or logs.
	 */
	...
	$effect(() => {
		const current = seats;
		if (
			passengers.seatIds.length !== current.length ||
	...
	const fieldErrors = $derived(
		passengers.entries.map((entry) => {
			if (!showErrors) return {} as Partial<Record<PassengerFieldError, string>>;
			const issues = validatePassenger(entry);
			const map: Partial<Record<PassengerFieldError, string>> = {};
			for (const issue of issues) map[issue.field] = resolveMessage(issue.messageKey);
			return map;
		})
	);

	const summaryIssues = $derived<SummaryIssue[]>(
		showErrors
			? passengers.entries.flatMap((entry, index) =>
					validatePassenger(entry).map((issue) => ({
	...
	function fieldSlug(field: PassengerFieldError): string {
		return field === 'fullName' ? 'name' : field;
	}

	function fieldLabel(field: PassengerFieldError): string {
		return {
	...
	const backHref = $derived(
		`/book/${data.busId}/seats${searchParams ? `?${searchParams}` : ''}`
	);
</script>
