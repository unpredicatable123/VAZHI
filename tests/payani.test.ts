import { SUGGESTED_TOPICS, matchTopic } from '../src/lib/services/payani.service';
import type { PayaniTopic } from '../src/lib/services/payani.service';
...
const topicOf = (q: string): PayaniTopic | null => matchTopic(q).topic;
...
check(
	'an action request outranks its own topic',
	topicOf('cancel my ticket') === 'cannot' && topicOf('how to cancel') === 'cancel'
);
...
check(
	'every chip resolves to its own topic',
	SUGGESTED_TOPICS.every((topic) => topic !== 'cannot')
);
