...
assert.equal(new Set(busFleetFixtures.map((bus) => bus.id)).size, busFleetFixtures.length, 'bus ids must be unique');
for (const [routeId, originStopId, destinationStopId] of corridors) {
    ...
    assert.ok(stopFixtures.some((stop) => stop.id === destinationStopId), `${destinationStopId} must be selectable`);
    const services = trips.filter((trip) => trip.routeId === routeId &&
    ...
    const offers = services.map((trip) => {
        ...
        assert.ok(crewFixtures.some((member) => member.id === trip.conductorId && member.role === 'conductor'));
        ...
        assert.equal(calculateFare(offer, 2).total, (offer.baseFare + offer.taxes) * 2, `${trip.code} must use the shared booking fare calculation`);
        ...
        assert.equal(deck.seats.filter((seat) => seat.availability === 'available').length, trip.seatsAvailable, `${trip.code} seat plan must match its availability count`);
        return offer;
    });
...
assert.equal(trips.filter((trip) => trip.boardingStopId === 'chennai-cmbt' &&
    trip.destinationStopId === 'madurai-mattuthavani' &&
    trip.sellable).length, 0, 'Chennai → Madurai remains deliberately unseeded');
for (const identifier of ['DRV-061', 'CON-061']) {
    assert.ok(demoAccounts.some((account) => account.identifier === identifier), `${identifier} needs seeded Firebase credentials`);
}
