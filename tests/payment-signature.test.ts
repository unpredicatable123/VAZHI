...
check(
	'ids that concatenate to the same string are not interchangeable',
	!verifySignature(SECRET, `${ORDER}|${PAYMENT}`, '', valid)
);
...
for (const [label, args] of [
	...
	let result = true;
	try {
		result = verifySignature(...(args as [string, string, string, string]));
	} catch {
		threw = true;
	}
